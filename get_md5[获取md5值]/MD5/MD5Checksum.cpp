...
RETURNS:		ATL::CString : the hexadecimal MD5 checksum for the specified file
ARGUMENTS:		ATL::CString& strFilePath : the full pathname of the specified file
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
ATL::CString CMD5Checksum::GetMD5(const ATL::CString& strFilePath)
{
	//open the file as a binary file in readonly mode, denying write access 
	//CAtlFile File;//(strFilePath, CAtlFile::modeRead | CAtlFile::shareDenyWrite | CAtlFile::typeBinary);
	//		HANDLE hFile = ::CreateFile(vPlayUrl.bstrVal, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,0);
	//CreateFile(pcszFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
	//File.Create(strFilePath.GetBuffer(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
*****************************************************************************************/
ATL::CString CMD5Checksum::GetMD5(CAtlFile& File)
{
	ATLASSERT(0);
	try
	{
...
RETURNS:		ATL::CString : the hexadecimal MD5 checksum for the specified data
...
NOTES:			Provides an interface to the CMD5Checksum class. Any data that can
				...
				function. Typically, ATL::CString and char arrays will be checksumed, 
...
*****************************************************************************************/
ATL::CString CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
RETURNS:		ATL::CString : the final hexadecimal MD5 checksum result 
ARGUMENTS:		None
...
*****************************************************************************************/
ATL::CString CMD5Checksum::Final()
{
	//Save number of bits
	BYTE Bits[8];
	...
	//Convert the hexadecimal checksum to a ATL::CString
	ATL::CString strMD5;
	for ( int i=0; i < nMD5Size; i++) 
	{
		ATL::CString Str;
		if (this->lpszMD5[i] == 0) {
			Str = ATL::CString(_T("00"));
		}
